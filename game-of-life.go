...
func drawCell(x, y int, drawBoard drawBoard) {
...
func main() {
	...
	counti := 0
	countj := 0
	drawBoard := makeDrawBoard(xrows, yrows, width, height)
	for !window.ShouldClose() {
		...
		if countj == yrows {
			countj = 0
			counti = (counti + 1) % xrows
		}
		glfw.PollEvents()
		window.SwapBuffers()
		fpsTime = glfw.GetTime() - fpsTime
		log.Println("FPS:", 1/fpsTime)
